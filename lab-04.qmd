...
```{r}
library(AOI)
palo_aoi <- geocode("Palo, Iowa", bbox = TRUE)
print(palo_aoi)
mapview(palo_aoi)
```
...
band_files <- list.files(
  path = "data/landsat/landsat-c2/",
  pattern = "*.TIF$", 
  full.names = TRUE, 
  recursive = TRUE) 

#Stack the band rasters  
rasters <- sort(band_files)[1:6]
stacked_rasters <- rast(rasters) |> 
setNames(bands)
```
...
```{r}
dims <- dim(stacked_rasters)
#Dimensions are 7801 by 7681 with 6 layers
crs(stacked_rasters)
#The CRS is WGS 84 UTM zone 15N or EPSG: 32615
res(stacked_rasters)
#Resolution is 30 by 30 m
```
...
```{r}
palo_crs <- st_transform(palo_aoi, crs = crs(stacked_rasters))
palo_crop <- crop(stacked_rasters, palo_crs)
```
...
```{r}
#Natural color
plotRGB(palo_crop, r = 4, g = 3, b = 2, stretch = "hist", 
        main = "Natural Color: Red-Green-Blue", mar = c(3, 3, 2, 1))
#Color infared 
plotRGB(palo_crop, r = 5, g = 4, b = 3, stretch = "hist", 
        main = "Color Infared: NIR-Red-Green", mar = c(3, 3, 2, 1))
#False color
plotRGB(palo_crop, r = 5, g = 6, b = 4, stretch = "hist", 
        main = "False Color, Water: SWIR-NIR-Red", mar = c(3, 3, 2, 1))
#My choice
plotRGB(palo_crop, r = 6, g = 3, b = 2, stretch = "hist", 
        main = "False Color, Bare Earth: SWIR-NIR-Red", mar = c(3, 3, 2, 1))
```
...
plot(indices_stack, col = colorRampPalette(c("blue", "white", "red"))(256))

```

> Describe the 5 images. How are they simular and where do they deviate?
ANSWER: SWI only shows places that are without a doubt water, meanwhile the other four show a channel and other features nearby. The two that best highlight the flooded area are MNDWI and WRI. MNDWI probably is the most accurate(or exagerated might be a better term) showing almost everything near the river channel and beyond as flooded.  
...
```{r}
ndvi_threshold <- app(ndvi, fun = function(x) ifelse(x < 0, 1, 0))
ndwi_threshold <- app(ndwi, fun = function(x) ifelse(x > 0, 1, 0))
mndwi_threshold <- app(mndwi, fun = function(x) ifelse(x > 0, 1, 0))
wri_threshold <- app(wri, fun = function(x) ifelse(x > 1, 1, 0))
swi_threshold <- app(swi, fun = function(x) ifelse(x < 5, 1, 0))
...
```{r}
threshold_stack <- c(ndvi_threshold, ndwi_threshold, mndwi_threshold, wri_threshold, swi_threshold) %>% 
  setNames(c("ndvi flood threshold", "ndwi flood threshold", "mndwi flood threshold", "wri flood threshold", "swi flood threshold"))

threshold_stack <- app(threshold_stack, fun = function(x) ifelse(is.na(x), 0, x))

plot(threshold_stack, col = colorRampPalette(c("white", "blue"))(256))
```
...
ANSWER:
All of the threshold flood maps look surprisingly similar. The MNDWI plot shows a few more blue "flood" pixels than the other 4 and the stream channel in NDWI appears to be smallest. Otherwise they all appear quite similar at first glance. 
...
```{r}
raster_values <- values(palo_crop)
dim(palo_crop)
```
...
```{r}
kmeans_12 <- kmeans(raster_values, centers = 12)
```
...
kmeans_raster <- palo_crop$nir08
#Set values of copied to the cluster
values(kmeans_raster) <- kmeans_12$cluster
plot(kmeans_raster, col = colorRampPalette(c("white", "blue", "red"))(256), main = "K-means Clustering (k=12)")

#Try different numbers of clusters
#Map them
kmeans_5 <- kmeans(raster_values, centers = 5)
values(kmeans_raster) <- kmeans_5$cluster 
plot(kmeans_raster, col = colorRampPalette(c("white", "blue", "red"))(256), main = "K-means Clustering (k=5)")

kmeans_10 <- kmeans(raster_values, centers = 10)
values(kmeans_raster) <- kmeans_10$cluster
plot(kmeans_raster, col = colorRampPalette(c("white", "blue", "red"))(256), main = "K-means Clustering (k=10)")

kmeans_20 <- kmeans(raster_values, centers = 20)
values(kmeans_raster) <- kmeans_20$cluster
plot(kmeans_raster, col = colorRampPalette(c("white", "blue", "red"))(256), main = "K-means Clustering (k=20)")
```
...
```{r}
flood_table <- table(values(threshold_stack$`mndwi flood threshold`), values(kmeans_raster))
flood_cluster <- which.max(flood_table[2,])
kmeans_flood <- app(kmeans_raster, fun = function(x) ifelse(x == flood_cluster, 1, 0))
plot(kmeans_flood, col = colorRampPalette(c("white", "blue"))(256), main = "Flood Classification with K-means")
```
...
```{r}
flooded_stack <- c(threshold_stack, kmeans_flood) |> 
  setNames(c("ndvi flood threshold", "ndwi flood threshold", "mndwi flood threshold", "wri flood threshold", "swi flood threshold", "kmeans flood threshold"))

flooded_cells <- global(flooded_stack, "sum", na.rm = TRUE)
...
```{r}
flood_certainty <- app(flooded_stack, fun = "sum", na.rm = TRUE)

plot(flood_certainty, 
     col = blues9, 
     main = "Flood Uncertainty Classifications")
```
...
```{r}
flood_mask <- flood_certainty
  
flood_mask[flood_mask == 0] <- NA

# 3. View it interactively
mapview(flood_mask, 
        col.regions = blues9, 
        na.color = "transparent",
        layer.name = "Flood Certainty Analysis")
```

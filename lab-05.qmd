...
# Question 1: Your Turn (15 points) #Where is the pdf?
...
```{r}
#Build the xgboost model and workflow
xg_model <- boost_tree() %>%
  set_engine("xgboost") %>%
  set_mode("regression")
...
xg_wf <- workflow() %>%
  ...
  add_model(xg_model) %>%
  # Fit the model
  fit_resamples(resamples = camels_cv) 

# Build the neural network model & workflow
nnet_model <- bag_mlp() %>%
  set_engine("nnet") %>%
  set_mode("regression")
...
#ANSWER: The best model based on the RMSE and RSQ values is the neural network model but because this is more of a "black box" model I will choose to use the second best model - the random forest model. The random forest model performs with only only slightly worse RSQ and RMSE than the neural network. 
```
...
# There are very few missing values in the data set, the only variable with missing values is the geology 2nd class variable. Variable types include precipitation variations like mean, seasonality, snow and frequency. A large array of geology characteristics is included as well as plenty about flow and runoff and flow high and low frequencies and duration data. At first glance, all of the data appears to be skewed either one way or the other. 

#Predictor variables 
#Choosing to leave out predictor attributes that use q-mean in their calculation because that would defeat the purpose. 
predictors <- c('q_mean', 'p_mean', 'pet_mean', 'elev_mean','area_gages2', 'aridity', 'frac_forest', 'soil_porosity')
cam <- camels |> 
  select(gauge_id, gauge_lat, gauge_lon, all_of(predictors))

skimr::skim(cam)
visdat::vis_dat(cam)
#Everything is skewed, frac_forest is also bimodal.
```

```{r}
#Lost
#cam_matrix <- as.matrix(cam)
...
# Generate the split
cam_split <- initial_split(cam, prop = 0.75)
cam_train <- training(cam_split)
cam_test  <- testing(cam_split)

cam_folds <- vfold_cv(cam_train, v = 10)
```
...
```{r}
# Create a recipe to preprocess the data
rec <-  recipe(logQmean ~ p_mean + pet_mean + elev_mean + 
               area_gages2 + aridity + frac_forest + soil_porosity, data = cam_train) |> 
  # Log transform the predictor variables (aridity and p_mean)
  step_log(p_mean, area_gages2, elev_mean, offset = 1) |> 
  # Normalize predicor variables 
...
```{r}
# Random Forest Model - Does will with non-linear relationships
rf_model <- rand_forest() %>%
  set_engine("ranger", importance = "impurity") %>%
  set_mode("regression")
...
xg_wf <- workflow() %>%
  ...
  add_model(xg_model) %>%
  # Fit the model
  fit_resamples(resamples = cam_folds) 

# Neural network model - The most black box option but the predictive power should be the best. 
nnet_model <- bag_mlp() %>%
  set_engine("nnet") %>%
  set_mode("regression")
...
```{r}
#First do the workflow for the regular folds 
wf <- workflow_set(list(rec),
                   list(rf_model,
                        xg_model,
                        nnet_model)) |> 
  workflow_map('fit_resamples', resamples = cam_folds)
...
```{r}
#Then do it again for the spatial blocks
library(spatialsample)
library(sf)

cam_train <- st_as_sf(cam_train, coords = c('gauge_lon', 'gauge_lat'), crs=4326) |>
  sf::st_set_crs(4326)

spatial_folds <- spatial_block_cv(cam_train)

wf_spatial <- workflow_set(list(rec),
                   list(rf_model,
                        xg_model,
                        nnet_model)) |> 
  workflow_map('fit_resamples', resamples = spatial_folds)

autoplot(wf_spatial) +
  theme_linedraw(18) +
  theme(legend.position = 'bottom')

rank_results(wf_spatial)
```
...
#The best model with the original fold split was the neural network (RSQ of 0.910) followed by the random forest. For the spatial split the best is the random forest with a lower RSQ of 0.841. This drop might seem small but given that the best models perform above a 0.9 its quite significant. I would choose to move forward with the random cv and Random Forest model due to the black box nature of the neural network and relative simplicity of the random forest. 
```
...
```{r}
#Tune the random CV Random Forest model.  
rf_model_tune <-
  rand_forest(trees = tune(), mtry = tune(), min_n = tune()) |>
  set_mode("regression") |>
  set_engine("ranger")

# Define the set of parameters you want to tune
...
rf_grid <- grid_space_filling(trees(range = c(200, 500)), 
                              min_n(range = c(2, 10)), 
                              mtry(range = c(1, 5)), 
                              size = 10)
...
autoplot(rf_grid_tune)
show_best(rf_grid_tune)

best_rf_tune <- select_best(rf_grid_tune, metric = "rsq")
print(best_rf_tune)
#The best model rmse & rsq has mtry of 5, trees = 266 and a min_n of 3. 
```
...
final <- workflow() |> 
  add_recipe(rec) |> 
  add_model(finalize_model(rf_model, best_rf_tune)) |> 
  fit(data = cam_train) 
...
#The top predictor is mean precipitation which makes sense because the amount of rain will most directly effect the stream discharge. The next variable is aridity, which is calculated using precipitation causing it to be a strong predictor. My next two are fraction forest and aridity x p_mean. After that is elevation mean.
```
...
final<- finalize_workflow(final, best_rf_tune)
final_fit <- last_fit(final, split = cam_split)
...
extract_workflow(final_fit) |> 
  augment(new_data = testing(cam_split)) |>
  ggplot(aes(x = logQmean, y = .pred, color = aridity)) + 
  geom_point(alpha = 0.8) + 
  scale_color_viridis_c() +
  geom_abline(lty = 2, color = "black", linewidth = 1) +
  labs(title = "Observed vs. Predicted Mean Discharge",
       x = "Log of Mean Discharge",
       y = "Predicted Log Mean Discarge ") +
  theme_classic()

#The model performs well with a RSQ of just over 0.9, which is high for something based on environmental/natural predictors. I was surprised that the spatial CV performed worse than the random CV. Where there is more data, like in regions with near average aridity, the model performs better. Outliers are scattered at the high and low ends. 
```

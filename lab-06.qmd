...
```{r}
g1 <- ggplot(hist, aes(x = as.Date(Date), y = Flow)) +
  geom_line(color = "blue") +
  labs(title  = "Monthly Streamflow — Cache la Poudre at Mouth (USGS 06752260)",
       y = "Flow (cfs)", x = "Date") +
  theme_minimal()

plot_ly(hist, x = ~as.Date(Date), y = ~Flow, type = "scatter", mode = "lines",
        line = list(color = "blue")) |>
  layout(title = "Monthly Streamflow — Cache la Poudre at Mouth(USGS 06752260)",
...
```{r}
gg_subseries(hist, y = Flow) +
  labs(title = "Monthly Flow Patterns — Cache la Poudre at Mouth",
       y = "Flow (cfs)", x = "Year") +
  theme_minimal()
```
...
```{r}
gg_season(hist, y = Flow) +
  labs(title = "Seasonal Shape by Year — Cache la Poudre at Mouth",
       y = "Flow (cfs)", x = "Month") +
  theme_minimal()
```
...
ANSWER: The first plot shows one panel for each month separated out which allows for quick analysis by looking at monthly mean over all years. The second seasonal plot shows each year plotted which allows for visualization in seasonal trends. The May to June peak is driven by snow-melt processes. 
...
gg_lag(log_hist, y = log_flow, geom = "point") +
...
ANSWER: Both the ADF and the KPSS tests indicate non-stationary. The lag plot also supports the same conclusion, the point clouds are dispersed. 
...
```{r}
decompose <- hist |>
  model(STL(Flow ~ trend(window = 14) + season(window = "periodic"))) |>
  components()
...
ANSWER: The Trend is overall decreasing but varies up and down, the highest highs were 2014-2016 and the lowest lows are 2020-2023. I would say that this is a clear decline. The seasonal componenet is driven by snowmelt in the early season then reservoir releases or summer storms that maintain slightly elevated flow post snowmelt peak. The remainder shows some positive spikes in 2013, 2014, and 2016. Otherwise negative troughs occur once yearly from 2018 until 2023. 
---
...
```{r}
set.seed(541)
splits <- time_series_split(hist_tbl, assess = "24 months", cumulative = TRUE)
...
ANSWER: The MARS EARTH model has the highest RSQ and lowest RMSE. This model also has a MASE of 0.454, lower than the rest; and a 13.3 MAPE, lower than the rest. The ARIMA-boost out performs the normal regression but the prophet-boost performs lowest of them all. The non-linear climate signal is extremely important for theis model to work well.
...
test_forecast <- calibration_table |>
  modeltime_forecast(new_data = select(testing, -Flow), actual_data = hist_tbl) |>
...
ANSWER: The predictions do track the actual values foe the timeframe specified. The confidence intervals do capture the observed values. All models predict below the actual for the discontinuity at the start of 2022.
...
```{r}
refit_tbl <- modeltime_refit(calibration_table, data = select(hist_tbl, -Flow))
...
ANSWER: The accuracy metrics do not change after refitting. 
...
future_forecast <- refit_tbl |>
  modeltime_forecast(new_data = future_tbl, actual_data = hist_tbl) |>
  mutate(.value   = expm1(.value),
         .conf_lo = expm1(pmax(.conf_lo, 0)),
         .conf_hi = expm1(.conf_hi))

...
```{r}
#This is the right code here use it below too. 
lees_annual <- annual_summary |> 
  filter(site_no == "09380000") |>
  filter(water_year < 2024) |>
  group_by(water_year, site_no) |> 
  summarise(mean_lees_flow = mean(total_annual_discharge)) |>
  ungroup()

glen_annual <- annual_summary |> 
  filter(site_no == "09085000") |>
    filter(water_year < 2024) |>
    rename(glenwood_flow = total_annual_discharge)

lees_glen <- lees_annual |> 
  left_join(glen_annual, by = "water_year")

#Check this, this is what needs fixing
nrow(lees_glen) == nrow(lees_annual)
...
```{r}
ggplot(aug, aes(x = Flow.x, y = Flow.y, color = season)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(title = "Predicted versus Actual Values for Lees Ferry",
       x = "Predicted Flow",
       y = "Actual Flow",
       color = "Season")
```
...
ggplot(aug, aes(x = .resid)) +
  ...
  labs(title = "Histogram of Lees Ferry Model Residuals",
       x = "Residuals",
       y = "Frequency")
...
> ...My join doesn't work how it should. I need to fix that before I can answer these questions because my plot and histogram do not plot the correct data right now. 

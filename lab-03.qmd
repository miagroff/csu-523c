...
```{r}
voronoi <- 
    st_cast(st_voronoi(counties)) |> 
    st_sf() |>
    mutate(id = 1:n()) 
  
triangles <-
    st_cast(st_triangulate(counties)) |> 
    st_sf() |> 
    mutate( id = 1:n()) 
...
tess_plot = function(data, title){
  ggplot() + 
    ...
    labs(title = title, 
         caption = paste0("Map of the US with ", nrow(data), " tile tesselation."))
    }
...
```{r}
plot_pip(voronoi_pip, "Voronoi Tesselation")
plot_pip(triangles_pip, "Triangles Tesselation")
plot_pip(hex_pip, "Hexagonal Tesselation")
plot_pip(grid_pip, "Counties Tesselation")
plot_pip(counties_pip, "Counties Tesselation")
```

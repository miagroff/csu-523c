...
```{r}
voronoi <- 
    st_cast(st_voronoi(counties_centroid)) |> 
    st_sf() |>
    mutate(id = 1:n()) 
  
triangles <-
    st_cast(st_triangulate(counties_centroid)) |> 
    st_sf() |> 
    mutate( id = 1:n()) 
 
grid <-
    st_make_grid(counties, n = c(70, 70)) |> 
    st_as_sf() |> 
    mutate(id = 1:n())
  
hex <- 
  st_make_grid(counties, square = FALSE, n= c(70,70)) |> 
    st_sf() |> 
    mutate(id = 1:n()) 
...
conus05 <- rmapshaper::ms_simplify(conus_border, keep = 0.05)
conus01 <- rmapshaper::ms_simplify(conus_border, keep = 0.01)
conus005 <- rmapshaper::ms_simplify(conus_border, keep = 0.005)
conus001 <- rmapshaper::ms_simplify(conus_border, keep = 0.001)

ggplot(conus_border) + 
  geom_sf() + 
  geom_sf(data = conus10, col = "red", fill = NA) +
  geom_sf(data = conus05, col = "orange", fill = NA) +
  geom_sf(data = conus01, col = "green", fill = NA) +
  geom_sf(data = conus005, col = "blue", fill = NA) +
  geom_sf(data = conus001, col = "purple", fill = NA) +
  theme_void()
#Green looks best going forward 
```
...
st_bbox(conus01)  # Should cover roughly [-125, 24] to [-66, 49]

# Check: Visual comparison
plot(conus_border, main = "Original")
plot(conus01, main = "Simplified")
```
:::
...
```{r}
#I removed around 11,000 points from 11,292 - or 99% of the points- from the map; this will speed up calculations because there are fewer iterations that need to run on a given calculation.
```
...
```{r}
voronoi_crop <- st_intersection(voronoi, conus01)
triangles_crop <- st_intersection(triangles, conus01)

grid_crop <- st_filter(grid, conus01)
hex_crop <- st_filter(hex, conus01)
```
...
tess_plot = function(data, title){
  ggplot() + 
    geom_sf(data = data, fill = "white", color = "navy", size = .2) + 
      theme_void() +
    theme(plot.title = element_text(face = "bold", color = "navy", hjust = .5),
          plot.caption = element_text(hjust = .5, color = "navy")) +
    labs(title = title, 
         caption = paste0("Map of the US with a ", nrow(data), " tile tesselation."))
    }
...
```{r}
tess_plot(voronoi_crop, "Voronoi Tesselation")
tess_plot(triangles_crop, "Triangle Tesselation")
tess_plot(grid_crop, "Grid")
tess_plot(hex_crop, "Hexagons")
...
```{r}
summarize_tess <- function(data, type) {
  #Convert and drop units
  area_km2 = drop_units(set_units(st_area(data), "km^2"))
  #Create dataframe
...
```{r}
summarize_tess(voronoi_crop, "voronoi")
summarize_tess(triangles_crop, "triangles")
summarize_tess(grid_crop, "gridded")
summarize_tess(hex_crop, "hexagonal")
summarize_tess(counties, "counties")
```
...
```{r, echo = FALSE}
summary_df_tess |> 
  knitr::kable(col.names = c("Tesselation", "Feature Count", "Mean Area (km²)", "SD Area", "Total Area (km²)"),
      digits = 2,
      caption = "Summary Statistics for CONUS Tessellations") %>%
...
```{r}
pip_fun = function (points, polygon, var){
...
```{r}
voronoi_pip <- pip_fun(dams2, voronoi, "id")
triangles_pip <- pip_fun(dams2, triangles, "id")
hex_pip <- pip_fun(dams2, hex, "id")
grid_pip <- pip_fun(dams2, grid, "id")
counties_pip <- pip_fun(dams2, counties, "id")
```
...
plot_pip <- function(data, title){
  ggplot() + 
    geom_sf(data = data, aes(fill = (n))) + 
    scale_fill_viridis_c() + 
    theme_void() + 
    theme(plot.title = element_text(face = "bold", color = "navy", hjust = .5, size = 16),
          plot.caption = element_text(face = "italic", color = "navy", hjust = .5, size = 10),
...
```{r}
voronoi_dams <- plot_pip(voronoi_pip, "Voronoi Tessellation")
triangles_dams <- plot_pip(triangles_pip, "Triangles Tessellation")
hex_dams <- plot_pip(hex_pip, "Hexagonal Tessellation")
grid_dams <- plot_pip(grid_pip, "Grid Tessellation")
counties_dams <- plot_pip(counties_pip, "Counties Tessellation")
```
...
combined_plot <- (voronoi_dams + triangles_dams + hex_dams) / (grid_dams + counties_dams + plot_spacer())
```
...
ANSWER: 
  Tessellation choice produces substantially different patterns
between all five maps. The most prominent differences occur are in the
hexagonal and grid tessellations there is a hotspot, with largest values
in the center of the US. The range of values also jumps to around
500-600 for these higher dam areas. In the counties and triangles
tessellations this hotspot doesn't occur as bright because they have
smaller area units near the central location. The triangles tessellation
happens to have a bright, high quantity area in Florida.
...
ANSWER: 
  For a dam operations manager seeking to understand geographic clustering I would recommend either the hexagonal or the grid tessellations as the best options. These options have equal areas throughout and thus spatially distribute and show the dam abundance per unit area. The other three tessellation options all rely on artificial boundaries and artificial county derived center points. None of these tessellations take into account natural boundaries such as watershed outlines or boundaries that would distinguish one drainage basin from the next. 
  It is important to understand management units though, such as state boundaries and county boundaries when considering the reasoning behind the decisions or analysis being done. The counties tessellation makes the most sense from a purely management perspective if there is an administrative question about county or state delegated functioning. I believe that each tessellation will be valid to answer different questions that are presented about the data.
  I believe that the question of hot spot versus jurisdictional boundary relies completely on the question being asked and who is asking it. For the US government any analysis about management would fall on some artificial boundary that has been drawn. On the other hand, if the question is about just the amount of dams per unit area for questions regarding spacing or river infrastructure the hexagonal and grid tessellations will answer questions better.
...
```{r}
irrigation <- dams2 |> 
  filter(grepl("Irrigation", purposes))
  
hydroelectric <- dams2 |> 
  filter(grepl("Hydroelectric", purposes))
  
navigation <- dams2 |> 
  filter(grepl("Navigation", purposes))
  
recreation <- dams2 |> 
  filter(grepl("Recreation", purposes))


pip_irrigation <- pip_fun(irrigation, hex_crop, "id")
pip_hydroelectric <- pip_fun(hydroelectric,hex_crop, "id")
pip_navigation <- pip_fun(navigation, hex_crop, "id")
pip_recreation <- pip_fun(recreation, hex_crop, "id")
```
...
```{r}
plot_pip(pip_irrigation, "Irrigation Dams") + gghighlight(n > (mean(n) + sd(n)))
plot_pip(pip_hydroelectric, "Hydroelectric Dams") + gghighlight(n > (mean(n) + sd(n)))
plot_pip(pip_navigation, "Navigation Dams") + gghighlight(n > (mean(n) + sd(n)))
plot_pip(pip_recreation, "Recreation Dams") + gghighlight(n > (mean(n) + sd(n)))
```
...
ANSWER: Irrigation dams are generally spread out in the Southwest and central plains regions. Hydroelectric dams are in the North East and in the mountains region of California. Navigation dams are dispersed throughout the East and along the Columbia River, recreational dams are found in the Southeast. This all makes some sense for the area, rivers, and needs of nearby communities. For example Irrigation being most common in the agricultural areas of the US. I am shocked and curious why recreational dams are so dominantly abundant in the Southeast and Eastern areas; I would assume for heat purposes to create lakes for swimming and cooling off during warm weather. 
...
aggr_dams <- st_join(hex_crop, dams2)
...
```{r}
map_1 <- ggplot(aggr_dams_calcs) +
  geom_sf(aes(fill = mean_year_completed), color = NA) +
  scale_fill_viridis_c() +
  theme_void() +
  labs(title = "Mean Year Completed by Tile") 

map_2 <- ggplot(aggr_dams_calcs) +
  geom_sf(aes(fill = sqrt(mean_surface_area)), color = NA) +
  scale_fill_viridis_c() +
  theme_void() +
  labs(title = "Sqrt Mean Surface Area by Tile") 

map_3 <- ggplot(aggr_dams_calcs) +
